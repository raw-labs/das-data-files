...
object HttpFileCache {
  ...
  lazy val global: HttpFileCache = {
    ...
    val cacheDirStr  = config.getString("myapp.httpFileCache.cacheDir") // e.g. "/tmp/httpCache"
    val cacheDir     = new File(cacheDirStr)

    val maxBytes     = config.getBytes("myapp.httpFileCache.maxCacheBytes") // e.g. "1g"
    val highFrac     = config.getDouble("myapp.httpFileCache.highWaterMarkFraction") // e.g. 0.75
    val lowFrac      = config.getDouble("myapp.httpFileCache.lowWaterMarkFraction")  // e.g. 0.50
  ...
  /**
   * Let the user programmatically build a new HttpFileCache from config keys,
   * if they prefer.
   */
  def fromConfig(configPath: String = "myapp.httpFileCache"): HttpFileCache = {
    val sub = config.getConfig(configPath)
    val cacheDirStr  = sub.getString("cacheDir")
    val cacheDir     = new File(cacheDirStr)

    val maxBytes     = sub.getBytes("maxCacheBytes")
    val highFrac     = sub.getDouble("highWaterMarkFraction")
    val lowFrac      = sub.getDouble("lowWaterMarkFraction")
