...
object SparkSessionBuilder {
  ...
  def build(appName: String, options: Map[String, String]): SparkSession = {
    ...
    options.get("aws_region").foreach(region => builder.config("fs.s3a.endpoint", s"s3.$region.amazonaws.com"))
    ...
    options.get("aws_access_key").foreach(accessKey => builder.config("fs.s3a.access.key", accessKey))
    options.get("aws_secret_key").foreach(secretKey => builder.config("fs.s3a.secret.key", secretKey))
